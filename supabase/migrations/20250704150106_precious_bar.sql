...
-- Tabela de usuários estendida (sem DEFAULT com subquery)
...
-- Função para definir role padrão
CREATE OR REPLACE FUNCTION set_default_user_role()
RETURNS trigger AS $$
DECLARE
  viewer_role_id uuid;
BEGIN
  -- Se role_id não foi fornecido, definir como 'viewer'
  IF NEW.role_id IS NULL THEN
    SELECT id INTO viewer_role_id FROM user_roles WHERE name = 'viewer' LIMIT 1;
    NEW.role_id := viewer_role_id;
  END IF;
...
-- Trigger para definir role padrão
...
-- Verificar se system_settings já existe antes de criar
DO $$
BEGIN
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'total_clients', (SELECT count(*) FROM clients WHERE COALESCE(status, 'active') = 'active'),
...
DROP POLICY IF EXISTS "Users can manage bookings based on role" ON bookings;
...
CREATE INDEX IF NOT EXISTS idx_clients_status ON clients(COALESCE(status, 'active'));
...
-- Inserir dados de exemplo para usuários (opcional)
DO $$
DECLARE
  admin_role_id uuid;
...
BEGIN
  ...
  SELECT id INTO operator_role_id FROM user_roles WHERE name = 'operator';
  SELECT id INTO viewer_role_id FROM user_roles WHERE name = 'viewer';
  
  -- Inserir usuário admin padrão se não existir
  INSERT INTO users (email, name, role_id, is_active) 
  VALUES ('admin@cleanpro.com', 'Administrador', admin_role_id, true)
